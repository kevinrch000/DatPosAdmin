...
function CargarTablaUsuario() {
    ...
    var obj = llenarobjeto('AdministrarUsuarios.php?action=ConsultarUsuarios');
    $('#hdd_numerofilas').val(obj.length);
    $('#table_id').DataTable({
        data: obj,
        columns: [
...
function ModalEmpresa() {
    ...
    obje = llenarobjeto('AdministrarUsuarios.php?action=TablaEmpresas');
...
function CargarTablaDetalleAccesos() {
    ...
    var obj = llenarobjeto('AdministrarUsuarios.php?action=MenuAccesos');
...
function Guardar() {
    ...
    $.ajax({
        type: "POST",
        url: 'AdministrarUsuarios.php?action=GrabarUsuario',
